...
gx.evt_i = (function($) {
				...
				var afterValidateFn = function () {
					if (gx.csv.invalidControl == null) {
						if (Ctrl.nodeName == 'SELECT') {
							...
							if (!gx.util.browser.isSafari() || (gx.dom.isTextControl(Ctrl) && Ctrl.selectionEnd === 0)) {
								gx.fn.setSelection(Ctrl);
							}
						}
					}
	...
	execEvt: function ( cmpCtx, inMaster, evt, ctrl, gridId, sync, srvCommand, disableForm, callback, force, failCallback) {
		...
		if (!force && ((gx.spa.isNavigating() || gx.evt.redirecting) || !gx.csv.validating && (gx.lang.emptyObject(evt) || gx.evt.processing ))) {
			callFailCallback();
			return;
		}
	...
	execCliEvt: function ( cmpCtx, inMaster, cliEvtName, evtGridName, gridRow, parms) {
		var deferred = $.Deferred();
		if (!gx.isInputEnabled() || (!gx.ajax.isFormEnabled() && !gx.csv.validating))
...
gx.csv_i =  (function($) {
	...
	return {
			...
			invalidateGXCtrl: function (Ctrl) {
				if (Ctrl) {					
					if ((Ctrl.getAttribute(gx.csv.GX_VALID_ATTRIBUTE) == 1) && gx.csv.controlValueHasChanged(Ctrl)) {
						gx.fn.setControlGxValid(Ctrl, "0");
					}
				}
			},
...
gx.http_i = (function ($) {
		...
		return {
			...
			setCookie: function (name, value, nDays, path) {
				path = (path)? ";path=" + path + ";": "";
				nDays = nDays || 1;
			...
			formatLink: function (url, parms) {
				...
				if (parms && parms.length > 0) {
					...
					for (var i = 0; i < len; i++) {
						...
						url += gx.http.encodeUriSegment(typeof(parms[i].getUrlVal) == 'function' ? parms[i].getUrlVal() : parms[i]);
					}
				}
				return url;
			},
...
(function($) {
	...
	prot.call = function (objUrl, args, target) {
	...
	prot.executeServerEvent = function (EvtName, Synch, EvtRow, Force, GlobalContextEvent) {
		...
		gx.evt.execEvt(this.CmpContext, this.IsMasterPage, evtName, gx.evt.dummyCtrl, gridId, synchReq, null, null, afterFnc, Force, afterFnc);
		return deferred.promise();
	};
	...
	var dispatchControlEvent = function (domEvt) {
		if (domEvt.type === "keypress" && (domEvt.keyCode !== 13)) { //Only enter Event (for keypress Evt) should be processed here.
			return; 
		}
				...
				if (!conditionCode || evaluateCode(conditionCode)) {
					...
					if (evtName) {
						...
						this.executeEvent(evtName, false, rowGxId, false, gridName).always(
							function(ctrlTarget) {
								if (!gx.spa.isNavigating() && !gx.evt.redirecting) {
									$(ctrlTarget).attr(gx.GxObject.GX_EVENT_EVENT_IN_PROGRESS, null);
								}
							}.closure(this, [$target])
						);
					}
	...
	prot.registerCtrlsEventHandlers = function () {
		...
		for (i=0, len=ctrlIds.length; i<len; i++) {
			...
			hasEvt = vStruct.evt || vStruct.evt2;
		...
		message: function (method, gxO, grid, row, serverSide, disableForm) {
			...
			return {
				...
				gridsData: function () {
					...
					var grids = {};
					if (grid) {
						var fnc = function(grid, grids, gxO, row) {
							$.each(grid.childGrids, function(i, cgrid) {
								var xgrid = gxO.getGridById(cgrid.gridId, row);
								if (!xgrid) {
									gx.dbg.write("gx.ajax.gridsData - grid not found for row: " + row, true);
									return;
								}
								var parentRow = (xgrid.parentRow)? xgrid.parentRow.gxId: "";
								grids[xgrid.gridName] = {
									id: xgrid.gridId,
									lastRow: xgrid.lastRowId,
									pRow: parentRow
								};
								fnc(xgrid, grids, gxO, row + xgrid.grid.GridRow);
							});
						}
						fnc(grid, grids, this.gxO, this.row);
					}
					else {
				...
				getInputParms: function () {
					...
					for (var i = 0, len = parmsMetadata.length; i < len; i++) {
						...
						else if (this.gxO.isTransaction() && this.grid > 0 && this.row && parm.av === 'Gx_mode' && parm.fld === 'vMODE')
						{
							...
							if (vStruct && typeof (vStruct.c2v) == 'function' && typeof (vStruct.val) == 'function' && vStruct.val(this.row) !== undefined) {
								vStruct.c2v(this.row);
							}
