	...
	this.render = function () {
		...
		if (!body) {
			...
			if ($(ucChild).hasClass('gx_usercontrol_child')) {
				...
				if (ucChildId.startsWith(containerId) || ucChildId.length > 5 && containerId.endsWith(ucChildId.substring(ucChildId.length - 5)) && ucChildId.startsWith(containerId.substr(0, containerId.length - 5))) {
					body = ucChild;
				}
			}
		}
								...
								if (thisC.ColumnsSelectorData_Aux.Columns != null && thisC.ColumnsSelectorData_Aux.Columns.length > 0) {
									...
									for (var k = 0; k < thisC.ColumnsSelectorData_Aux.Columns.length; k += 1) {
										if (thisC.ColumnsSelectorData_Aux.Columns[k].C == ek) {
											...
											$(this).css("background-image", "url(" + thisC.control.DropDownOptionsTitleSettingsIcons[isSelected ? "MultiselOption_GXI" : "MultiselSelOption_GXI"] + ")");
											break;
										}
									}
								}
